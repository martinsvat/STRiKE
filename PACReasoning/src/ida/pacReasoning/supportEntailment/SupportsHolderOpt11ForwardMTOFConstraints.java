 ...
 * <p>
 * Follows 10ForwadMoreThanOneFix with constraints.
 * <p>
 * Created by martin.svatos on 15. 10. 2018.
 */
public class SupportsHolderOpt11ForwardMTOFConstraints {
    ...
    private long ones = 0;
    //private long carts;
    private final ConcurrentHashMap<Literal, BSet> headLiteralToBitSet;
    ...
    public SupportsHolderOpt11ForwardMTOFConstraints(int k, Set<Literal> evidence, Map<Constant, BSet> constantToSubset, List<Constant> evidenceConstants, Map<Literal, Set<BSupport>> map, Map<BSet, BSupport> supportCache, Map<Pair<String, Integer>, Integer> predicateToIdx, boolean useExtraPruning, Map<Constant, Integer> constantToInteger, List<Clause> negatedConstraints) {
        ...
        this.headLiteralToBitSet = new ConcurrentHashMap<>();
    ...
    public void forwardRules(List<Triple<Clause, Literal, BSet>> rulesWithNegated, long iteration, Set<Triple<Clause, Literal, BSet>> newlyAddedRules) {
        ...
        System.out.println("forbidden\t" + this.forbidden.size());
...
//        String debugStr = "manages(health_care_related_organization, group)";//"manages(health_care_related_organization, disease_or_syndrome)";
        String debugStr = "";//"complicates(neoplastic_process, experimental_model_of_disease)";//"manages(health_care_related_organization, disease_or_syndrome)";
//        String debugStr = "prevents(medical_device, cell_or_molecular_dysfunction)";

        boolean literalRemovedInLastIteration = false;
        ...
        for (int currentK = 1; currentK <= k; currentK++) {
            ...
            do {
                ...
                // check
                for (Map.Entry<Literal, Set<BSupport>> literalSetEntry : this.map.entrySet()) {
                    if (null == this.map.get(literalSetEntry.getKey())) {
                        System.out.println(literalSetEntry);
                        throw new IllegalStateException();
                    }
                    if (literalSetEntry.getValue().isEmpty()) {
                        System.out.println(literalSetEntry);
                        throw new IllegalStateException();
                    }
                }
...
//                System.out.println("map\t" + map.keySet().size() + "\t" + sizeOfSupports);
                ...
                for (Triple<Clause, Literal, BSet> preparedRule : rulesWithNegated) {
                    ...
                    if (firstIterationOfNewRule || isWorthTrying(preparedRule, changedPredicates, globalChangedPredicates, currentIteration)) {
                        world = completelyNewLiterals.isEmpty() && !literalRemovedInLastIteration ? world : Matching.create(new Clause(map.keySet()), Matching.THETA_SUBSUMPTION);

//                        System.out.println("world is " + (completelyNewLiterals.isEmpty() && !literalRemovedInLastIteration ? "old" : "new"));
...
//                                        .map(x -> this.toCanon(x))
                ...
                // incorporating minimal supports
                for (Map.Entry<Literal, Set<BSet>> entry : changesToBeAdded.entrySet()) {
                    ...
                    minimals = forbidenFree(minimals);
                    ...
                    if (null == this.map.get(head)) {
                        if (debugThisIteration) {
                            System.out.println("no support before, add all!\t" + minimals.size());
                        }
                        ...
                        toRemove.forEach(support -> support.removeHead(head));
                        newFiltered.forEach(support -> support.addLiteral(head));
                    ...
                    if (debugThisIteration) {
                        ...
                        this.map.get(head).stream().map(x -> toCanon(x) + x.getSet().stream().mapToObj(z -> "" + z).collect(Collectors.joining(", "))).sorted().forEach(i -> System.out.println(":\t" + i));
                    }
                ...
                // constraints
                literalRemovedInLastIteration = false;
                List<Clause> relevantConstraints = selectRelevantConstraints(currentlyChangedPredicates);
                ...
                Set<BSupport> removeSupports = literalsWithChangedSupports.values().stream().flatMap(Collection::stream).distinct()
                        .filter(support -> {
                            Matching subworld = Matching.create(new Clause(mask(support)), Matching.THETA_SUBSUMPTION);
                            for (Clause constraint : relevantConstraints) {
                                if (subworld.subsumption(constraint, 0)) {
                                    // wanna remove this support
                                    // check
                                    if (support.getSet().cardinality() < finalCurrentK) {
                                        throw new IllegalStateException();
                                    }
                                    return true;
                                }
                ...
                //System.out.println("remove supports\t" + removeSupports.size() + "\t" + relevantConstraints.size());
...
//                    Set<Literal> nonRemoved = ConcurrentHashMap.newKeySet();
                    for (Literal literal : support.getLiterals()) {
                        /*if (evidence.contains(literal)) { I think this is not needed... 'cause it's just wrong to have this literal there if it's causing inconsistencies
                        ...
                        boolean debugHere = literal.toString().equals(debugStr);


                        if (evidence.contains(literal)) {
                            System.out.println("throwing out evidence literal\t" + literal);
                        }
                        Set<BSupport> sups = map.get(literal);
                        sups.remove(support);
                        if (sups.isEmpty()) {
                            map.remove(literal);
//                            System.out.println("removing literal from map!\t" + literal);
                            literalRemovedInLastIteration = true;
                            if (completelyNewLiterals.containsKey(literal)) {
                                completelyNewLiterals.remove(literal);
                            }
                        }


                        if (debugHere) {
                            System.out.println("here I'm\t" + literal);
                            if (null != map.get(literal)) {
                                map.get(literal).stream().map(this::toCanon).forEach(System.out::println);
                            }
                            System.out.println("-");
                        }
...
//                        support.setLiterals(nonRemoved);
    ...
    private List<BSet> forbidenFree(List<BSet> minimals) {
    ...
    private boolean isForbidden(BSet set) {
        for (BSupport support : this.forbidden) {
            if (set.isSubsetOf(support.getSet())) {
                return true;
            }
        }
        return false;
    ...
    private List<Clause> selectRelevantConstraints(BitSet changedPredicates) {
        List<Clause> retVal = Sugar.list();
        for (Clause negatedConstraint : this.negatedConstraints) {
            for (Literal literal : negatedConstraint.literals()) {
                ...
                if (changedPredicates.get(this.predicatesToIdx.get(literal.getPredicate()))) {
                    retVal.add(negatedConstraint);
                    break;
                }
            }
        }
...
//        }
    ...
    public static SupportsHolderOpt11ForwardMTOFConstraints create(Set<Literal> evidence, int k, Map<Pair<String, Integer>, Integer> predicatesToBitset, boolean useExtraPruning, List<Clause> negatedConstraints) {
        ...
        return new SupportsHolderOpt11ForwardMTOFConstraints(k, evidence, constantToSubset, constants, map, //heads,
...
//        }
    ...
    private Iterable<? extends Literal> mask(BSupport support) {
        return map.entrySet().parallelStream()
                .filter(entry -> {
                    if (!getBitset(entry.getKey()).isSubsetOf(support.getSet())) {
                        return false;
                    }
                    return entry.getValue().stream().anyMatch(literalSupport -> literalSupport.getSet().isSubsetOf(support.getSet()));
                }) // bude tohle fungovat spravne?
                .map(Map.Entry::getKey)
                .collect(Collectors.toList());
    }

    private BSet getBitset(Literal literal) {
        BSet retVal = this.headLiteralToBitSet.get(literal);
        if (null == retVal) {
            retVal = constantsToSubset(literal);
            this.headLiteralToBitSet.put(literal, retVal);
        }
        return retVal;
    }
