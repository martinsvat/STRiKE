 ...
 */
public class SupportsHolderOpt3 {
    ...
    private Set<Literal> changedLiteralsInLastIteration;
...
//        System.out.println("SupportsHolder -- todo: namisto metody mask pouzit nejakou jinou datovou strukturu aby i support mel seznam literalu ktere uvozuje");
    ...
    public SupportsHolderOpt3(int k, Set<Literal> evidence, Map<Constant, Subset> constantToSubset, List<Constant> evidenceConstants, Map<Literal, Set<Support>> map, Map<Subset, Support> supportCache, Map<Pair<String, Integer>, Integer> predicatesIdx) {
        ...
        this.changedLiteralsInLastIteration = ConcurrentHashMap.newKeySet();
        System.out.println("tady je chyba v tom changedPredicatesInLastIteration protoze se to drzi a neaktualizuje :)), takze i v constraints filtru je to spatne");
...
//                }
                ...
                if (canUsePruning) {
                    for (Literal literal : groundBody.literals()) {
                        ...
                        if (this.changedLiteralsInLastIteration.contains(literal)) {
                            allUntouched = false;
                            break;
                        }
                    }
...
//                    }
                    ...
                    if (beVerbouse) {
                        System.out.println(head + "\t" + groundBody.hashCode() + "\t" + (null == previousSupports ? -1 : previousSupports.size()) + "\t" + minimals.size());
...
//        }
    ...
    public static SupportsHolderOpt3 create(Set<Literal> evidence, int k, Map<Pair<String, Integer>, Integer> predicatesToBitset) {
        ...
        return new SupportsHolderOpt3(k, evidence, constantToSubset, constants, map, //heads,
...
//            System.out.println("adding to forbiddens\t" + toCanon(support));
            ...
            for (Literal literal : support.getLiterals()) {
                ...
                //
                // this.changedLiteralsInLastIteration.add(literal); tohle je uplne zbytecne tady :))
            }
