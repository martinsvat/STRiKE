 ...
 */
public class SupportsHolderOpt5Backup {
    ...
    // speeding up?
    private final Map<Integer, Set<Support>> forbiddens;
...
//        System.out.println("SupportsHolder -- todo: namisto metody mask pouzit nejakou jinou datovou strukturu aby i support mel seznam literalu ktere uvozuje");
    ...
    public SupportsHolderOpt5Backup(int k, Set<Literal> evidence, Map<Constant, Subset> constantToSubset, List<Constant> evidenceConstants, Map<Literal, Set<Support>> map, Map<Subset, Support> supportCache, Map<Pair<String, Integer>, Integer> predicatesIdx, boolean useExtraPruning) {
        this.k = k;
            ...
            for (Term[] terms : subts.s) {
                ...
                */
                for (Literal literal : preparedRule.r.literals()) {
                    System.out.println("!!! this clashes with groundBody!");
                    if (SpecialVarargPredicates.SPECIAL_PREDICATES.contains(literal.predicate()) || SpecialBinaryPredicates.SPECIAL_PREDICATES.contains(literal.predicate())) {
                        continue;
                    }
...
//        debugIt2();
    ...
    private Set<Support> toMinimalForbiddenFreeSupports(Set<BitSet> rawSupports) {
        ...
        List<BitSet> nonForbidden = rawSupports.stream()
                ...
                .sorted(Comparator.comparing(BitSet::cardinality)) // TODO tohle tady chybělo
    ...
    private boolean isForbiddenS(Subset subset) {
        ...
        if (this.oldVersion) {// old version
            for (Support support : forbidden) {
                ...
                if (support.getSubset().isSubsetOf(subset)) {
                    return true;
                }
            }
            return false;
         ...
         /**/
            ...
            // speedup version ?
        /* */
            int atMostSize = subset.size();
            for (Integer integer : subset.values()) {
                Set<Support> possibleSubsets = this.forbiddens.get(integer);
                if (null != possibleSubsets) {
                    for (Support support : possibleSubsets) {
                        if (support.size() <= atMostSize && support.getSubset().isSubsetOf(subset)) {
                            return true;
                        }
                    }
                }
    ...
    private boolean isForbiddenB(BitSet bitset) {
        ...
        /**/
            Subset subset = SubsetFactory.getInstance().get(bitset); // ok, this may be overhead over the old version
            int atMostSize = bitset.size();
            for (Integer integer : subset.values()) {
                Set<Support> possibleSubsets = this.forbiddens.get(integer);
                if (null != possibleSubsets) {
                    for (Support support : possibleSubsets) {
                        if (support.size() <= atMostSize) {
                            this.forbiddenBCallsWithin++;
                            if (support.getSubset().isSubsetOf(subset)) {
                                this.forbiddenBStops++;
                                return true;
...
//        }
    ...
    public static SupportsHolderOpt5Backup create(Set<Literal> evidence, int k, Map<Pair<String, Integer>, Integer> predicatesToBitset, boolean useExtraPruning) {
        ...
        return new SupportsHolderOpt5Backup(k, evidence, constantToSubset, constants, map, //heads,
        ...
        */
        ...
        for (Support support : removeSupports) {
            if (this.oldVersion) {// old version
                ...
                // speedup version
            /**/
                if (null == this.forbiddens.get(support.getSubset().getLowestValue())) {
                    this.forbiddens.put(support.getSubset().getLowestValue(), Sugar.set());
                }
                this.forbiddens.get(support.getSubset().getLowestValue()).add(support);
                this.forbiddensSize++;
            /**/
            }
