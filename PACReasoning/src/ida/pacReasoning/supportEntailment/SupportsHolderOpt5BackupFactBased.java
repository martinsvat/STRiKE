 ...
 */
public class SupportsHolderOpt5BackupFactBased {


    private final Map<Literal, Subset> factToSubset;
    private final Map<Subset, Literal> subsetToFact;
    private final Map<Integer, Literal> idxToFact;
    private final Map<Set<Literal>, Subset> factsToSubset;
...
//        System.out.println("SupportsHolder -- todo: namisto metody mask pouzit nejakou jinou datovou strukturu aby i support mel seznam literalu ktere uvozuje");
    ...
    public SupportsHolderOpt5BackupFactBased(int k, Set<Literal> evidence, Map<Literal, Subset> factToSubset, Map<Literal, Set<Support>> map, Map<Subset, Support> supportCache, Map<Pair<String, Integer>, Integer> predicatesIdx, boolean useExtraPruning) {
        this.k = k;
        this.evidence = evidence;
        this.factToSubset = factToSubset;
        this.subsetToFact = new HashMap<>();
        this.idxToFact = new HashMap<>();
        factToSubset.forEach((key, value) -> {
            this.subsetToFact.put(value, key);
            List<Integer> list = value.getBitset().stream().mapToObj(i -> i).collect(Collectors.toList());
            if (list.size() != 1) {
                throw new IllegalStateException();
            }
            Integer idx = list.get(0);
            this.idxToFact.put(idx, key);
        });
        this.factsToSubset = new HashMap<>();
        this.map = map;
        ...
        for (Pair<Clause, Literal> preparedRule : rulesWithNegated) {
            ...
            for (Term[] terms : subts.s) {
                ...
                */
                ...
                if ((allUntouched && atLeastOneNonEvidence && map.containsKey(head))){// || isForbiddenT(groundBody)) {
                    continue;
                }
                // 2 alerts!
//                    ground literal in body does not have to be inside basic dictionary! (null pointer; just skip that one)
                    //  -> and also take care its supports
                    //how to resolve forbidden of literals, because we cannot simply take it based on constants, we need it based on literals, but what about subset relation as is done now, will it work?
                    //  -> yes, this will work
    ...
    private String toCanon(Support s) {
        return "{" + factToSubset.entrySet().stream()
    ...
    // this does not give sense in fact-based support since we would have to do cartesian over all literals in the ground body
//    private boolean isForbiddenT(List<Literal> terms) {
//
...
//            System.out.println("vstup 2\t" + supports.size() + "\t\t" + supports + "\n\t\t" + previous.size() + "\t\t" + previous);
  ...
  /*return retVal.stream().map(bitset -> {
    ...
    public Map<Literal, Set<Support>> getSupports() {
    ...
    public KECollector entailed() {
        ...
        //SupportCollector retVal = SupportCollector.get(this.factToSubset);
//        try {
//
...
//        }
    ...
    public static SupportsHolderOpt5BackupFactBased create(Set<Literal> evidence, int k, Map<Pair<String, Integer>, Integer> predicatesToBitset, boolean useExtraPruning) {
        Map<Literal, Subset> factToSubset = new HashMap<>();
        List<Literal> evidenceList = Sugar.listFromCollections(evidence);
        IntStream.range(0, evidenceList.size()).forEach(idx -> factToSubset.put(evidenceList.get(idx), SubsetFactory.getInstance().get(evidenceList.size(), idx)));
        ...
        for (int idx = 0; idx < evidenceList.size(); idx++) {
            Literal literal = evidenceList.get(idx);
            Subset subset = SubsetFactory.getInstance().get(evidenceList.size(), idx);
            Support support = supportCache.get(subset);
            if (null == support) {
                support = Support.create(subset);
                supportCache.put(subset, support);
        ...
        return new SupportsHolderOpt5BackupFactBased(evidence.size() + 1, evidence, factToSubset, map, //heads,
...
//        Subset retVal = factsToSubset.get(constants);
//        if (null != retVal) {
//            return retVal;
//        }
//        retVal = SubsetFactory.getInstance().union(constants.stream().map(c -> factToSubset.get(c)).collect(Collectors.toList()));
//        factsToSubset.put(constants, retVal);
//        return retVal;
//    }
...
//    }
    ...
    public boolean constraintsCheck(List<Clause> negatedConstraints, long iteration, List<Clause> predictiveHornRules) {
        ...
        if (!filteredNegatedConstraints.isEmpty()) {
            ...
            checkAdding(filteredNegatedConstraints, Sugar.set(), 0 == iteration, predictiveHornRules);
        }
    ...
    private void checkAdding(List<Clause> negatedConstraints, Set<Support> testingBecauseNewHeads, boolean checkAllSupports, List<Clause> predictiveHornRules) {
...
//        Set<Support> supports = this.map.values().stream().flatMap(Collection::stream).collect(Collectors.toCollection(LinkedHashSet::new)); // tak timhle to ocividne neni
        */
        Set<Support> removeSupports = this.map.values().stream()
                ...
                .filter(support -> {
                    ...
                    // this can be speed-up by some smart caching
                    Matching world = Matching.create(new Clause(mask(support, predictiveHornRules)), Matching.THETA_SUBSUMPTION);
        ...
        for (Support support : removeSupports) {
            ...
            for (Literal literal : support.getLiterals()) {
                if (evidence.contains(literal)) {
                    ...
                    /* TODO tohle musi byt jeste upraveno ale nevim ted jak
                    if (support.getSet().isSubsetOf(constantsToSubset(literal))) { // in this case, constants(literal) is equal to support
    ...
    // there should be a check on predicitveHornRules if they are really Horn ;)
    private Iterable<? extends Literal> mask(Support support, List<Clause> predictiveHornRules) {
        LeastHerbrandModel herbrand = new LeastHerbrandModel();

        List<Literal> currentEvidence = unfold(support); // support.literals() should be the ones that are derived on the next line
        return herbrand.herbrandModel(predictiveHornRules, currentEvidence); // this work is maybe redundant (see how support.literals() works)
    ...
    private List<Literal> unfold(Support support) {
        return support.getSubset().getBitset().stream().mapToObj(idx -> this.idxToFact.get(idx)).collect(Collectors.toList());
    }
    ...
    public List<BitSet> setsToUnions(List<Set<Support>> supports) {
        ...
        for (Set<Support> support : supports) {
            BitSet bs = new BitSet(this.factToSubset.size());
    ...
    public BitSet toUnion(Set<BitSet> bts) {
        BitSet retVal = new BitSet(this.factToSubset.size());
        for (BitSet bt : bts) {
            retVal.or(bt);
        }
        return retVal;
