 ...
 * and with batch (so this should be pretty much correct than 6, 7 & 8).
 * <p>
 * Created by martin.svatos on 15. 10. 2018.
 */
public class SupportsHolderOpt9ForwardBatch {
...
//        String debugStr = "manages(health_care_related_organization, group)";//"manages(health_care_related_organization, disease_or_syndrome)";
        String debugStr = "";//"contains(gene_or_genome, body_substance)";//"manages(health_care_related_organization, disease_or_syndrome)";
//        String debugStr = "prevents(medical_device, cell_or_molecular_dysfunction)";

//        Counters<Set<Set<BSet>>> counter = new Counters<>();
        Cache<Set<Set<BSet>>, MultiList<Integer, BSet>> cache = new Cache<>();
...
//                System.out.println("map\t" + map.keySet().size() + "\t" + sizeOfSupports);
                ...
                MultiMap<Literal, Set<Set<BSet>>> changesToMake = new MultiMap<>();
...
//                                        .map(x -> this.toCanon(x))
                            ...
                            changesToMake.put(head, fold(groundBody, preparedRule, 1 == updatedLiterals ? lookup : null));
                        }
                    }
                }

                // changes incorporation
                for (Map.Entry<Literal, Set<Set<Set<BSet>>>> entry : changesToMake.entrySet()) {
                    Literal head = entry.getKey();


                    //nekde je tu bug uz na zacatku :(

                    Set<BSet> candidates = Sugar.set();
                    for (Set<Set<BSet>> bodiesSupports : entry.getValue()) {
//                        counter.increment(bodiesSupports);
                        //one possibility
                        //candidates.addAll(cartesianProduct(bodiesSupports, currentK));
                        // cache variant
                        MultiList<Integer, BSet> cached = cache.get(bodiesSupports);
                        if (null == cached) {
                            cached = new MultiList<>();
                            for (BSet bSet : cartesianProductWithMax(bodiesSupports, k)) {
                                cached.put(bSet.cardinality(), bSet);
                            }
                            cache.put(bodiesSupports, cached);
                        }
                        candidates.addAll(cached.get(currentK));
                    }
        ...
        //counter.toMap().values().stream().forEach(val -> histogram.increment(val));
        //System.out.println("histogram");
        //int total = histogram.keySet().stream().mapToInt(histogram::get).sum();
        //String distributon = histogram.keySet().stream().sorted().map(x -> "" + (1.0 * histogram.get(x) / total))
//                .collect(Collectors.joining(", "));
    ...
    private Set<Set<BSet>> fold(List<Literal> groundBody, Triple<Clause, Literal, BSet> preparedRule, MultiMap<Literal, BSupport> pruneLookup) {
        Set<Set<BSet>> retVal = new LinkedHashSet<>(groundBody.size() + (0 == preparedRule.t.cardinality() ? 0 : 1));
        if (0 < preparedRule.t.cardinality()) {
            retVal.add(Sugar.set(preparedRule.t));
        }

        for (Literal literal : groundBody) {
            Set<BSupport> supports = null != pruneLookup && pruneLookup.containsKey(literal) ? pruneLookup.get(literal) : supports(literal);
//            Set<BSet> current = Sugar.set();
//            for (BSupport support : supports) {
//                current.add(support.getSet());
//            }
//            retVal.add(current);
            retVal.add(supports.stream().map(BSupport::getSet).collect(Collectors.toSet()));
        }
        return retVal;
    }

    private Set<BSet> cartesianProduct(Set<Set<BSet>> bodiesSupports, int currentK) {
        Set<BSet> previousSupports = null;
        for (Set<BSet> literalSupports : bodiesSupports) {
            Set<BSet> next = Sugar.set();
            if (null == previousSupports) {
                next = literalSupports;
            } else {
                for (BSet set : literalSupports) {
                    for (BSet previousSet : previousSupports) {
                        BSet union = previousSet.union(set);
        ...
        return previousSupports.stream().filter(set -> currentK == set.cardinality()).collect(Collectors.toSet());
    }

    private Set<BSet> cartesianProductWithMax(Set<Set<BSet>> bodiesSupports, int k) {
        Set<BSet> previousSupports = null;
        for (Set<BSet> literalSupports : bodiesSupports) {
            Set<BSet> next = Sugar.set();
            if (null == previousSupports) {
...
//        }
    ...
    public static SupportsHolderOpt9ForwardBatch create(Set<Literal> evidence, int k, Map<Pair<String, Integer>, Integer> predicatesToBitset, boolean useExtraPruning) {
        ...
        return new SupportsHolderOpt9ForwardBatch(k, evidence, constantToSubset, constants, map, //heads,
