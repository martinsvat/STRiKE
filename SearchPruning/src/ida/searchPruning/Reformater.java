...
public class Reformater {
    ...
    public static void main(String[] args) throws IOException {
        //formate();
        //count();
        datasetStats(Paths.get("..","datasets","nci","gi50_screen_KM20L2.txt"));
    }

    private static void datasetStats(Path path) throws IOException {
        MEDataset dataset = MEDataset.create(path.toString(), Matching.THETA_SUBSUMPTION);
        System.out.println("examples\t" + dataset.getExamples().size());
        System.out.println("literals\t" + dataset.getExamples().stream().mapToInt(Clause::countLiterals).sum());
    }
