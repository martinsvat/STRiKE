...
public class Crossvalidation {
    ...
    public Stats<List<HornClause>> run(Searchable<List<HornClause>> search, int folds) {
        List<Integer> indices = IntStream.range(0, dataset.getExamples().size()).boxed().collect(Collectors.toList());
    ...
    public static  Pair<Set<Integer>, Set<Integer>> split(int idx, int step, int folds, List<Integer> indices) {
        if (folds < 2) {
    ...
    private void mergeResults(List<LevelWiseStats> result, BreadthResults current, Set<Integer> train, Set<Integer> test) {
        for (int depth = 0; depth < current.depths(); depth++) {
            ...
            addTo(current.getDepth(depth),
                    current.getRules(depth),
                    current.getReducedFeatures(depth),
                    result.get(depth),
                    train,
                    test);
        }
