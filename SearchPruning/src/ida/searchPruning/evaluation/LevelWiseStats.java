...
public class LevelWiseStats {
    ...
    public Double avgHardRules() {
        return Utils.average(Utils.longToDouble(this.sameLevel.stream().map(levelStats -> levelStats.getHardRules()).collect(Collectors.toList())));
    }

    public Double avgMinSuppRules() {
        return Utils.average(Utils.longToDouble(this.sameLevel.stream().map(levelStats -> levelStats.getMinSuppHardRules()).collect(Collectors.toList())));
    }
}
