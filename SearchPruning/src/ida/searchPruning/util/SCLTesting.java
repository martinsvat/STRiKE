...
//        EnhancedShortConstraintLearner enhanced = EnhancedShortConstraintLearner.create(med, maxLength, maxVariables,0);
//        List<Pair<Clause, Double>> constraintsExtendedSupport = enhanced.learnConstraints();
        List<Pair<Clause,Double>> constraintsExtendedSupport = Sugar.list();
        List<Clause> constraintsExtended = constraintsExtendedSupport.stream()
                .filter(pair -> 0 == Double.compare(0.0d, pair.s))
                .map(pair -> pair.r)
                .collect(Collectors.toList());
